...
| **Runtime-proven?** | **Yes, fully.** Real `HitmFighterRuntime` (Module 5A) + real `HitmAssetImporter`/`BuildSpriteDrawData` (Module 5B) both succeed end to end — live-verified via `dominus-cli hitm-sprite-draw-data`, exact real elapsed-frame values reproduced at every attack sub-state boundary. | **Asset + draw-data layers proven; full gameplay runtime not.** `HitmAssetImporter::Import` succeeds (atlas/parts/anim/rig all real, valid, cross-checked); `BuildSpriteDrawData` now also proven against his own real idle/walk/jump snapshots and his own real, distinct secondary-motion spring constants (stiffness=0.268/damping=0.784, vs. Brooklyn's 0.118/0.634), including an exact frame-by-frame differential proof (`test_hitm_sprite_draw_data_multi_fighter.cpp`). What's still not provable: `HitmFighterRuntime::Create`/`HitmMoveInstance::Extract` fail on his real, incomplete move data (Module 5A's own pre-existing finding, unchanged) — confirmed live: `dominus-cli hitm-sprite-draw-data` on Rocket fails at move extraction, never at asset import — so no attack/hitstun draw-data sequence, and no *simulated* (vs. hand-constructed) idle/walk sequence, has been run for him. | **Asset + draw-data layers proven; full gameplay runtime not.** Same shape as Rocket, including his own distinct real secondary-motion constants (stiffness=0.184/damping=0.652) proven via the same exact differential methodology. Full runtime gameplay sequence still not provable until his own move-schema gap is separately addressed (out of this report's scope). |
